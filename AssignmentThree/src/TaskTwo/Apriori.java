 ...
 */
class Apriori extends Observable {
     ...
     * 
     */
    Apriori(String transaFile, double minSup, int iteration) throws Exception
    {
        configure(transaFile,minSup,iteration);
        go();
    }
    ...
    private void go() throws Exception {
        ...
        while (itemsets.size()>0 && iteration>0)
        {
...
//        System.out.println(Arrays.toString(itemset) + " - ("+ ((support / (double) numTransactions))+" "+support+")");
        ArrayList<Integer> temp=result.get(support);
        if (temp==null){
            result.put(support,convertToList(itemset));
        }else{
            temp.addAll(convertToList(itemset));
            result.put(support,temp);
        }
    }
    ...
    private void configure(String itransaFile,double iminSup, int iiteration) throws Exception
    {        
        if (iminSup>1 || iminSup<0){
            throw new Exception("minSup: bad value");
        }
        transaFile=itransaFile;
        minSup=iminSup;
       ...
       for(int i=0; i<itemsets.size(); i++)
       {
           for(int j=i+1; j<itemsets.size(); j++)
           {
               ...
               if (ndifferent==1) {
                   ...
                   // I have to create the hash myself using a String :-(// I use Arrays.toString to reuse equals and hashcode of String
                   Arrays.sort(newCand);
   ...
   Map<Integer, ArrayList<Integer>> getResult() {
       return result;
   }
